...
class Admin::CappuccinoController < ApplicationController   
  ...
  def show_cappuccino_ui
    @menu = "mlayout"
    user_id = Mytemplate.get(params[:doc_name].to_i).user_id
    userid = User.get(user_id).userid
    @userid = userid
  ...
  def post_mlayout
    ...
    path = "#{RAILS_ROOT}/public/user_files/" + @user.userid + "/article_templates/" + mytemplate.file_filename.gsub(/.zip/,'')
    xml_file= <<-EOF
  ...
  def request_mlayout        
    ...
    docname = params[:docname] 
    temp_id = docname.gsub(".mlayoutP","").to_i
    userinfo = params[:userinfo] 
    id = docname
        
    mytemplate = Mytemplate.get(temp_id)
    @user = User.get(mytemplate.user_id)
  ...
  def filelist
    puts_message $user_id.to_s
    puts_message "filelist start"   
    
    user = User.get($user_id)
    puts_message "사용자 아이디::::::::::::" + user.userid
...
#----------------------------------------------------
  def upload_notify
    ...
    if params[:userid] && params[:path]
      path = "#{RAILS_ROOT}" + "/public/user_files/" + "#{$userid}"+ "/article_template/"+ params[:path]
